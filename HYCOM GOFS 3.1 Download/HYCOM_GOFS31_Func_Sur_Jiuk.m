function HYCOM_GOFS31_Func_Sur_Jiuk(Period, timestep, Spatial_area, lat_step, lon_step, VariableData, HomeFolderNm, RawDataFolderNm, MatFileNm, HYC_ver)
...
%           GLBy0.08/expt_93.0_sur.html?dataset=GLBy0.08-expt_93.0-sur
% -------------------------------------------------------------------------
...
if datestart >= HycSetTime(1,1) && dateend <= HycSetTime(1,2) && datestart <= dateend
    % continue
else
    disp('Wrong INPUT date range! \n');        
end

% check Variables - "qtot", "emp", "steric_ssh", "ssh", "u_bt", "v_bt", "sblt", "mld"
varcheck    = nan(1,8);
varcheck(1) = ismember("qtot", string(VariableData));
varcheck(2) = ismember("emp", string(VariableData));
varcheck(3) = ismember("steric_ssh", string(VariableData));
varcheck(4) = ismember("ssh", string(VariableData));
varcheck(5) = ismember("u_bt", string(VariableData));
varcheck(6) = ismember('v_bt', string(VariableData));
varcheck(7) = ismember('sblt', string(VariableData));
varcheck(8) = ismember('mld', string(VariableData));

idlinkfull = "https://tds.hycom.org/thredds/dodsC/GLBy0.08/expt_93.0/sur";

ads1 = idlinkfull;
% daterange_str_end = [daterange(1), daterange(end)];
...
% hyc_down_specific_area ==================================================
    ...
    function hyc = hyc_down_specific_area(ads1, id_tm, id_la, id_lo, varcheck, lat_step, lon_step)
        ...
        for i_var = 1:length(varcheck)
            if varcheck(i_var) == 1
                if i_var == 1
                    adsset1 = sprintf('%s,qtot%s',adsset1, adsrng2);
                elseif i_var == 2
                    adsset1 = sprintf('%s,emp%s',adsset1, adsrng2);
                elseif i_var == 3
                    adsset1 = sprintf('%s,steric_ssh%s',adsset1, adsrng2);
                elseif i_var == 4
                    adsset1 = sprintf('%s,ssh%s',adsset1, adsrng2);
                elseif i_var == 5
                    adsset1 = sprintf('%s,u_barotropic_velocity%s',adsset1, adsrng2);
                elseif i_var == 6
                    adsset1 = sprintf('%s,v_barotropic_velocity%s',adsset1, adsrng2);
                elseif i_var == 7
                    adsset1 = sprintf('%s,surface_boundary_layer_thickness%s',adsset1, adsrng2);
                elseif i_var == 8
                    adsset1 = sprintf('%s,mixed_layer_thickness%s',adsset1, adsrng2);
                end
            end
        end
        ...
        for i_var = 1:length(varcheck)
            if varcheck(i_var) == 1
                if i_var == 1
                    hyc.qtot = downloadretry(ads_f1, "qtot");
                elseif i_var == 2
                    hyc.emp = downloadretry(ads_f1, "emp");
                elseif i_var == 3
                    hyc.steric_ssh = downloadretry(ads_f1, "steric_ssh");
                elseif i_var == 4
                    hyc.ssh = downloadretry(ads_f1, "ssh");
                elseif i_var == 5
                    hyc.u_baro = downloadretry(ads_f1, "u_barotropic_velocity");
                elseif i_var == 6
                    hyc.v_baro = downloadretry(ads_f1, "v_barotropic_velocity");
                elseif i_var == 7
                    hyc.sblt = downloadretry(ads_f1, "surface_boundary_layer_thickness");
                elseif i_var == 8
                    hyc.mld = downloadretry(ads_f1, "mixed_layer_thickness");
                end
